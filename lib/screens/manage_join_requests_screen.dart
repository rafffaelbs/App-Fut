...
class _ManageJoinRequestsScreenState extends State<ManageJoinRequestsScreen> {
  ...
  Future<void> _approve(Map<String, dynamic> request, {String? existingPlayerId}) async {
    ...
    if (_processingIds.contains(requestId)) return; // já está processando, ignora clique duplicado
    ...
    try {
      ...
      // Remove localmente na hora, sem esperar o _load(), pra não dar
      // brecha de clicar de novo enquanto a lista ainda não recarregou.
      if (mounted) setState(() => _requests.removeWhere((r) => r['id'].toString() == requestId));
      if (mounted) {
      ...
      final message = e.toString().contains('duplicate key')
          ? 'Essa solicitação já tinha sido aprovada.'
          : 'Erro: $e';
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _reject(Map<String, dynamic> request) async {
    ...
    setState(() => _processingIds.add(requestId));
    try {
      await SupabaseService.instance.groupMembers.rejectRequest(requestId);
      if (mounted) setState(() => _requests.removeWhere((r) => r['id'].toString() == requestId));
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
